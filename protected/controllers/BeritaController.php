...
class BeritaController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Berita']))
		{
			...
			$model->gambar='tes.jpg';
			
			if($model->save())
				//$this->redirect(array('view','id'=>$model->id));
			{
				if(CUploadedFile::getInstance($model,'gambar'))
					{
						$newfilename=''.$model->id.'.jpg';
						$model->gambar=CUploadedFile::getInstance($model,'gambar');
						$model->gambar->saveAs(Yii::getPathOfAlias('webroot').'/images/'.$newfilename);
						$model->gambar=$newfilename;
						$model->save();
					}
					if(CUploadedFile::getInstance($model,'gambar'))  // check if uploaded file is set or not
					{
						//todo
					}
				...
				if(!empty ($uploadedFile))
					{
						...
						$var = '';
						$model->gambar = $var.'/images/img'.$fileName;
						$model->save();
	...
	{
		$db=new Berita('search');
		$db->unsetAttributes();  // clear any default values
		if(isset($_GET['Berita']))
			$db->attributes=$_GET['Berita'];

		$model = Berita::model()->findAll();
